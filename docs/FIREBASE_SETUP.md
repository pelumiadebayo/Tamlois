...
10. Seed active services, service extras, service intake schemas and all active policies. Save public-safe availability and payment settings through the admin.
...
The included rules deny all public booking and hold reads/writes; the Admin SDK in Cloud Functions bypasses those rules only after server validation. `businessSettings/public` intentionally exposes operating rules and reason-free blocks, while private operational records remain admin-only. Before production, enable App Check or equivalent request attestation and rate limiting, configure retention for intake data, and test cancellation cleanup and concurrent submissions against the emulator.
